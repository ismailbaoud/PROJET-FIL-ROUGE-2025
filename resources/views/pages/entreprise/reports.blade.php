                ...
                <div class="max-w-7xl mx-auto">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <button
                                class="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all">
                            ...
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div class="flex flex-wrap gap-3">
                                    <div class="relative">
                                        <select name="program_id" onchange="document.getElementById('filterForm').submit()"
                                            class="appearance-none pl-3 pr-8 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-full md:w-48">
                                    ...
                                    <div class="relative">
                                        <select name="severity" onchange="document.getElementById('filterForm').submit()"
                                            ...
                                            <option value="Critical"
                                                {{ request('severity') == 'Critical' ? 'selected' : '' }}>Critical</option>
                                        </select>
                                        <div
                                            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                            <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <div class="relative">
                                        <select name="status" onchange="document.getElementById('filterForm').submit()"
                                            ...
                                            <option value="triaged" {{ request('status') == 'triaged' ? 'selected' : '' }}>
                                                Triaged</option>
                                            <option value="duplicate"
                                                {{ request('status') == 'duplicate' ? 'selected' : '' }}>Duplicate</option>
                                            <option value="informative"
                                                {{ request('status') == 'informative' ? 'selected' : '' }}>Informative
                                            </option>
                                            <option value="not_applicable"
                                                {{ request('status') == 'not_applicable' ? 'selected' : '' }}>Not
                                                Applicable</option>
                                            <option value="not_reproducible"
                                                {{ request('status') == 'not_reproducible' ? 'selected' : '' }}>Not
                                                Reproducible</option>
                                            <option value="resolved"
                                                {{ request('status') == 'resolved' ? 'selected' : '' }}>Resolved</option>
                                            <option value="wont_fix"
                                                {{ request('status') == 'wont_fix' ? 'selected' : '' }}>Won't Fix</option>
                                ...
                                <div class="relative w-full md:w-64">
                                    <input type="text" name="search" value="{{ request('search') }}"
                                        placeholder="Search reports..." class="..."
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        <div
                            class="bg-white rounded-xl shadow-xs border border-gray-200 p-4 hover:shadow-md transition-all">
                            ...
                            <div class="flex justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-2xl font-bold text-green-600">
                                        {{ $reports->where('status', 'Accepted')->count() }}</p>
                                </div>
                                    ...
                                    @foreach ($reports as $report)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-500">
                                                {{ $report->id }}</td>
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-semibold text-gray-900">{{ $report->title }}
                                                </div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div
                                                        class="h-5 w-5 rounded-full bg-blue-100 flex items-center justify-center mr-2">
                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                            class="h-3 w-3 text-blue-600" viewBox="0 0 20 20"
                                                            fill="currentColor">
                                                            <path fill-rule="evenodd"
                                                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707  Mosk 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                                                clip-rule="evenodd" />
                                                        </svg>
                                                    </div>
                                                    <span
                                                        class="text-sm text-gray-900">{{ $report->program->title ?? 'Unknown' }}</span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @php
                                                ...
                                                <span
                                                    class="px-2 py-1 inline-flex text-xs leading-4 font-bold rounded-full {{ $severity['bg'] }} {{ $severity['text'] }} uppercase">
                                                    {{ $report->severity }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                @endphp
                                                <span
                                                    class="px-2 py-1 inline-flex text-xs leading-4 font-medium rounded-full {{ $statusStyles }}">
                                                    {{ $report->status }}
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                <div class="flex items-center">
                                                    ...
                                                    {{ $report->reward ? '$' . number_format($report->pointe    , 0) : 'Pending' }}
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end space-x-3">
                                                    ...
                                                    <form action="{{ route('entreprise_report_update', $report->id) }}"
                                                        method="POST" id="status-form-{{ $report->id }}">
                                                        @csrf
                                                        @method('PUT')
                                                        <select name="status"
                                                            ...
                                                            <option value="new"
                                                                {{ $report->status == 'new' ? 'selected' : '' }}>New
                                                            </option>
                                                            <option value="triaged"
                                                                {{ $report->status == 'triaged' ? 'selected' : '' }}>
                                                                Triaged</option>
                                                            <option value="duplicate"
                                                                {{ $report->status == 'duplicate' ? 'selected' : '' }}>
                                                                Duplicate</option>
                                                            <option value="informative"
                                                                {{ $report->status == 'informative' ? 'selected' : '' }}>
                                                                Informative</option>
                                                            <option value="not_applicable"
                                                                {{ $report->status == 'not_applicable' ? 'selected' : '' }}>
                                                                Not Applicable</option>
                                                            <option value="not_reproducible"
                                                                {{ $report->status == 'not_reproducible' ? 'selected' : '' }}>
                                                                Not Reproducible</option>
                                                            <option value="resolved"
                                                                {{ $report->status == 'resolved' ? 'selected' : '' }}>
                                                                Resolved</option>
                                                            <option value="wont_fix"
                                                                {{ $report->status == 'wont_fix' ? 'selected' : '' }}>Won't
                                                                Fix</option>
                                                            <option value="spam"
                                                                {{ $report->status == 'spam' ? 'selected' : '' }}>Spam
                                                            </option>
                                                        </select>
                                                    </form>
                                                </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between mt-6">
                        <div class="text-sm text-gray-500 mb-4 sm:mb-0">
                            Showing <span class="font-medium">{{ $reports->firstItem() }}</span> to <span
                                class="font-medium">{{ $reports->lastItem() }}</span> of <span
                                class="font-medium">{{ $reports->total() }}</span> reports
                        </div>
